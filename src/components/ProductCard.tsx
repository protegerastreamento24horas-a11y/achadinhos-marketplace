...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className="group block bg-white border border-gray-100 hover:border-[#FF4B2B]/30 transition-all hover:shadow-lg hover:shadow-[#FF4B2B]/5 flex flex-col h-full"
    >
      {/* Image */}
      <div className="relative overflow-hidden bg-gray-50 aspect-square">
        {discount > 0 && (
          <div className="absolute top-0 left-0 z-10 bg-[#FF4B2B] text-white text-[10px] font-bold px-2 py-1">
            -{discount}%
          </div>
        )}
        {badgeLabel && (
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        <h3 className="text-sm font-semibold text-gray-900 line-clamp-2 mb-2 group-hover:text-[#FF4B2B] transition-colors leading-snug">
          {product.title}
        </h3>
          ...
          <div className="flex items-center gap-1 mb-2">
            <div className="flex">
              {Array.from({ length: 5 }).map((_, i) => (
                <svg key={i} className={`h-3 w-3 ${i < Math.round(product.rating!) ? "text-amber-400" : "text-gray-200"}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="mt-auto pt-2 border-t border-gray-50">
          <div className="flex items-baseline gap-2 flex-wrap">
            <span className="text-base font-bold text-[#FF4B2B]">{product.price}</span>
